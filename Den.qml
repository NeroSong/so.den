...
BarWidget {
  ...
  Item {
    ...
    Repeater {
      // Keep slots stable while the registry unregisters widgets one by one.
      // A filtered array model rebuilds every remaining child on each change.
      // Each slot's Loader already tracks its own component availability.
